...
describe("the rows", () => {
  ...
  it("⛔ describe each occupant by its PRODUCT, never by where its name came from", () => {
    // This read `display_name_source` and printed `bakta_symbol` under every row on the real page.
    const descriptions = mountPopover().findAll(".alt-desc").map((node) => node.text());
    expect(descriptions).toContain("O-antigen ligase");
    expect(descriptions).not.toContain("bakta_symbol");
  });
