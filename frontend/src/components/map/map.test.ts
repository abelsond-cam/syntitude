...
function neighbour(ordinal: number, overrides: Partial<NeighbourDisplayRow> = {}): NeighbourDisplayRow {
  return {
    ...
    map_position: { bacformer: null, esm: null },
    genome_count: 50,
...
describe("⛔ one picture — the whole-catalogue view was removed (David, 2026-09-22)", () => {
  it("offers no zoom and draws no catalogue: the fit of these six is the whole card", () => {
    // *"It isn't helpful. Just display closest 6 neighbours."* Pinned so that bringing the catalogue
    // picture back has to fail a test first, rather than slipping in with a refactor.
    const card = mountMap();
    expect(card.find(".zooms").exists()).toBe(false);
    expect(card.find("image").exists()).toBe(false);
    expect(card.findAll("svg")).toHaveLength(1);
    expect(card.text()).not.toContain("whole catalogue");
  });
});
