...
/**
 * **The neighbourhood map — a FIT of these six, not a crop of a big one.**
 *
 ...
 * UMAP preserves local neighbourhoods, not global distance, and no caption rescues a crop of one.
 * So this is classical MDS on the six loci's own pairwise Euclidean distances, fitted per locus.
 *
 ...
 * picture is asked. Nothing is rescaled to make mid-range rings visible: most loci are far tighter
 * than their nearest rival and that *is* the finding.
 *
 ...
 *
 * ⛔ **SVG, not canvas.** The published page drew this on a canvas alongside the whole-catalogue
 * dust, which genuinely needs raster at 889k points. Six dots and six rings do not, and jsdom
 * computes no canvas — so a canvas version of this is invisible to every test, which is the same
...
/** The fit, or `null` where there are too few measured points to make one. */
const fit = computed(() => {
  ...
  return {
    ...
    points: drawn.map((member, index) => {
      ...
      return {
        ...
        colour: member.isFocal ? "var(--map-focal)" : (HUES[(index - 1) % HUES.length] as string),
      };
    }),
  };
});

/** The legend: one row per drawn locus, with its cosine to the focal one. */
